...
export function reportInvalidEscapeError(state: ParserState, type: InvalidEscapeType): void {
  switch (type) {
    ...
    case InvalidEscapeType.InvalidHex: report(state, Errors.Unexpected); // falls through
    case InvalidEscapeType.OutOfRange: report(state, Errors.Unexpected); // falls through
    default: return;
  }
}
